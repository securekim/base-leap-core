...
const value = BigInt('99');
const color = 32900;

const spendCond1 = Tx.spendCond(
  [new Input({
    prevout: new Outpoint(prevTx, 0),
    gasPrice: 12345,
    msgData: 0,         // target (IoT Device)
    script: 4,          //policy
  })], [new Output(value, ADDR, color)],    // value is refered as 'tokenId'
);

const unspentSC = [
  {output: spendCond1.outputs[0], outpoint: new Outpoint(spendCond1.hash(), 0)},
];

// Cryptonian.BASE 테스트 코드 추가
describe('Spending Conditions', () => {
 
  it('should allow to create and parse spending condition tx with new API..', () => {
    const prevTx = '0x7777777777777777777777777777777777777777777777777777777777777777';
    const value = BigInt('99000000');
    const color = 1337;
    
    const inputs = makeSCInputs(unspentSC, ADDR, 0, 0, 5, color);   // tokenId 0, target 0, policy 5
    const outputs = makeSCOutputs(inputs, ADDR, 0, color);          // tokenId 0

    Tx.spendCond(inputs, outputs);
    
  });

  // Cryptonian - Test for Deposit with States.
  it('should allow to create Tx.Deposit with state..', () => {
      const color = 32900;
      const depositId = 3;
      const tokenId = 123;
      const state = 6;
      ...
      const depositStateHash = '0x37b8e4b39f5c85b2441a2af61078d671a5a751865f5699c67fc7200ef28a62b8';

      const deposit = Tx.depositState(depositId, ADDR, color, tokenId, target, state);
      // toJSON
      const json = deposit.toJSON();
      expect(json).to.eql({
        type: Type.DEPOSIT,
        hash: depositStateHash,
        inputs: [{msgData:target, script:state}],
        outputs: [{address:ADDR, color:color, value:tokenId}],
        options: {depositId},
      });
      //fromJSON
...
describe('transactions', () => {
  ...
  it('should allow to serialze and deserialize depositState tx to/from Raw.', () => {
    const depositHash = '0xd6974543796b6f469c4b4d2e5db3670c9ef721655595c2db894dbbdcf45ac824';
    const value = BigInt('1');
    const color = 32769;
    const depositId = 1;
    //// static depositState(depositId, address, color, tokenId, target, state) // from Cryptonian..
    const deposit = Tx.depositState(depositId, ADDR, color, value, "1","0");

    // toJSON
    var raw = deposit.hex();
    var temp = Tx.fromRaw(raw).toJSON();

    console.log("hex/fromRaw/toJSON");
    console.log(temp);

    expect(temp).to.eql({
      type: Type.DEPOSIT,
      inputs: [],
      outputs: [{ address: ADDR, value: value, color: color, storageRoot:'{"script":"0x000","msgData":"123"}' }],
      options: { depositId },
    });
    
    // fromJSON
    assert.deepEqual(Tx.fromRaw(raw), deposit);
  });
...
// Cryptonian.BASE - Commented out : Error 발생 : --> 'ReferenceError : regeneratorRuntime is not defined' 에러 발생
/*
