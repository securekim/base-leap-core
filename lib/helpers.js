...
export function formSCInputs (unspent, from, to, newPolicy, color) {
...
// Cryptonian
export function formSCOutputs(unspent, inputs, from, to, color) {
  if (unspent.length === 0) {
    throw new Error ('Unspent is Empty');
  }

  // Check Inputs?!..
  const outputs = [new Output(BigInt(to), from.toLowerCase(), color)];
