...
describe('helpers', () => {
  ...
  // Cryptonian - for testing spending_condition
  // ADDR_1 : Token's Owner
  // ADDR_2 : IoT Device's Address..
  const SC_TokenID = 100;
  const SC_Color = 12345;
  const depositState1 = Tx.depositState(0, ADDR_1,SC_Color, SC_TokenID, ADDR_2.toString(), "0x777" );
  const unspentState = [
    { output: depositState1.outputs[0], outpoint: new Outpoint(depositState1.hash(), 0)},
  ];
/*
  const deposit1 = Tx.deposit(1, 100, ADDR_1);
  const deposit2 = Tx.deposit(2, 200, ADDR_1);
...
*/
  ...
  // Cryptonian
  describe('formSCInputs',() => {
    it('should return inputs with exact parameter', () => {
      var prevState = JSON.parse (unspentState[0].output.storageRoot);
      expect(prevState.msgData == ADDR_2.toString());
      expect(prevState.script == "0x777");

      const inputs1 = formSCInputs(unspentState, ADDR_1, SC_TokenID, ADDR_2.toString(), "0x444", SC_Color);
      expect(inputs1.length).to.equal(1);
      // expect(inputs1[0].msgData).to.equal(ADDR_2.toString());
      expect(inputs1[0].msgData == ADDR_2.toString());
      expect(inputs1[0].script == "0x444");
    });
  });

  // Cryptonian
  describe('formSCOutputs', () => {

    it('sholud return outputs with exact parameter', () => {
      const inputs1 = formSCInputs(unspentState, ADDR_1, SC_TokenID, ADDR_2.toString(), "0x444", SC_Color);
      const outputs1 = formSCOutputs(inputs1, ADDR_1, SC_TokenID, SC_Color);
      expect(outputs1.length).to.equal(1);
      var state = JSON.parse (outputs1[0].storageRoot);
      // expect(state.msgData).to.equal(ADDR_2.toString());
      expect(state.msgData == ADDR_2.toString());
      expect(state.script == "0x444");
