...
describe('Output', () => {
  ...
  it('should allow to decode raw transfer output (NFT)', () => {
    const output = new Output({ value: tokenId, color: ERC721color, address: ADDR, storageRoot:'{"script":"0x000","msg":"123"}' }); // Cryptonian
    assert.deepEqual(
      output,
  ...
  it('should allow to hex/decode raw transfer output (NFT)', () => {
    const output = new Output({ value: tokenId, color: ERC721color, address: ADDR, storageRoot:'{"script":"0x000","msg":"123"}' }); // Cryptonian
    assert.deepEqual(
      output,
      Output.fromRaw(output.toRaw()),
