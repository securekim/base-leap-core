...
//export const OUT_LENGTH = 86; // Cryptonian + 32 bytes - storageRoot
...
export default class Output {
  ...
  toJSON() {
    const rsp = {
      ...
      color: this.color,
      storageRoot : this.storageRoot, // Cryptonian
    };
    return rsp;
  }
  ...
  static fromRaw(buf, offset = 0) {
    ...
    //Cryptonian 
    const storageRoot = `${buf.slice(offset + 54, offset + 86).toString()}`; //'hex'
    console.log("Cryptonian-output - toRaw");
    console.log(storageRoot);
    var tempOutput = new Output(value,address,color);
    tempOutput.storageRoot = storageRoot;
    // transfer output
  ...
  toRaw() {
    ...
    var dataBuf;  
    if (this.storageRoot) {
      dataBuf = Buffer.alloc(this.getSize() + this.storageRoot.toString().length);
    } else {
      dataBuf = Buffer.alloc(this.getSize());
    }
    //const dataBuf = Buffer.alloc(this.getSize()); // storageRoot "0xabcd" --> +2!!
    ...
    console.log("Cryptonian-output - toRaw");
    if (this.storageRoot) {
      console.log(this.storageRoot);
      //dataBuf.write(this.storageRoot.replace('0x', ''), 54, this.storageRoot.toString().length); // address 20 bytes
      dataBuf.write(this.storageRoot, 54, this.storageRoot.toString().length); // address 20 bytes
    }
