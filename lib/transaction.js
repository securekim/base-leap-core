...
export default class Transaction {
  ...
  // Cryptonian - added deposit 'states'  // State를 Deposit 할 경우 필요한데..
    // depositId, depositor, color, tokenId, target, state
    // tokenId 가 기존 ERC20 일때의 value와 매칭된다..
  static depositState(depositId, address, color, tokenId, target, state) {

    var tmpOutput = new Output(tokenId, address, color);
    tmpOutput.storageRoot = JSON.stringify({"script": state, "msgData":target});

    return new Transaction(
      //Type.DEPOSIT, [new Input({msgData:target, script:state})], [new Output(tokenId, address, color)], {depositId},
      //Type.DEPOSIT, [new Input({msgData:target, script:state})], [tmpOutput], {depositId},
      Type.DEPOSIT, [], [tmpOutput], {depositId}, // Cryptonian.. empty Input
    );
  }
